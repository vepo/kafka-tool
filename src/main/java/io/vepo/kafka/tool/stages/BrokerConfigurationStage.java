...
public class BrokerConfigurationStage extends AbstractKafkaToolStage {
    ...
    public BrokerConfigurationStage(Stage owner) {
        ...
        dataTableRef[0] = gridBuilder.newLine()
                                   ...
                                   .editable(TextFieldTableCell.<KafkaBroker>forTableColumn(),
                                             (broker, value) -> applyTableEdit(broker,
                                   ...
                                   .button("Delete",
                                           broker -> {
                                               ...
                                               clearValidation(validationLabel, txtName, txtBootstrapServers,
                                                       txtSchemaRegistryUrl);
                                               refreshAddButtonState(txtName, txtBootstrapServers, txtSchemaRegistryUrl,
                                                       btnAdd, brokers);
                                           })
        ...
        Runnable refreshAddButtonState = () -> refreshAddButtonState(txtName,
                txtBootstrapServers,
                txtSchemaRegistryUrl,
                btnAdd,
                brokers);
        ...
        btnAdd.setOnAction(e -> {
            ...
            var result = KafkaBrokerValidator.validate(broker, brokers);
            if (!result.valid()) {
                showValidation(validationLabel, broker, brokers, result.message(), txtName, txtBootstrapServers,
                        txtSchemaRegistryUrl);
                return;
            }
    ...
    private static KafkaBroker draftBroker(TextField txtName, TextField txtBootstrapServers,
        ...
        return new KafkaBroker(txtName.getText().trim(),
                               txtBootstrapServers.getText().trim(),
                               txtSchemaRegistryUrl.getText().trim());
    }

    private static void refreshAddButtonState(TextField txtName, TextField txtBootstrapServers,
            TextField txtSchemaRegistryUrl, Button btnAdd, List<KafkaBroker> brokers) {
        btnAdd.setDisable(!KafkaBrokerValidator.validate(draftBroker(txtName, txtBootstrapServers, txtSchemaRegistryUrl),
                                                         brokers)
                .valid());
    }

    private static void applyTableEdit(KafkaBroker broker, Runnable applyChange, Label validationLabel,
            TableView<KafkaBroker> dataTable, List<KafkaBroker> brokers) {
        ...
        if (!result.valid()) {
            ...
            Platform.runLater(() -> {
                showValidation(validationLabel, broker, brokers, result.message(), null, null, null);
                dataTable.refresh();
            });
            return;
        }

        Settings.updateKafka(kafka -> kafka.setBrokers(brokers));
        Platform.runLater(() -> clearValidation(validationLabel, null, null, null));
    }

    private static void showValidation(Label validationLabel, KafkaBroker broker, List<KafkaBroker> brokers,
        ...
        if (broker != null) {
            setFieldInvalid(txtName,
                            !KafkaBrokerValidator.validateName(broker.getName(), broker, brokers).valid());
