...
public class KafkaAdminService implements Closeable {
        ...
        executor.submit(() -> {
            ...
            if (nonNull(adminClient)) {
                ...
                handle(adminClient.describeTopics(asList(topic.name()))
                                  .all(),
                       this::listOffsets, error -> logger.error("Error describing topic!", error));
            }
        });
    }
    ...
    public void listBrokers(Consumer<List<Broker>> callback) {
        executor.submit(() -> {
            if (nonNull(adminClient)) {
                try {
                    callback.accept(adminClient.describeCluster()
                                               .nodes()
                                               .get()
                                               .stream()
                                               .map(node -> new Broker(node.id(),
                                                                       node.host(),
                                                                       node.port(),
                                                                       node.rack()))
                                               .toList());
